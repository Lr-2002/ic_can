...
void print_usage(const char *program_name) {
  ...
  std::cout << "Real-time angle monitoring for wrist motors (m7-m8)"
            << std::endl;
  std::cout << std::endl;
  std::cout << "Usage: " << program_name << " [options]" << std::endl;
  std::cout << "Options:" << std::endl;
  std::cout << "  -f <freq>     Update frequency in Hz (default: 20)"
            << std::endl;
  ...
  std::cout << "  " << program_name
            << " -f 30 -l     # Monitor at 30Hz with logging" << std::endl;
  std::cout << "  " << program_name
            << " -t 60        # Monitor for 60 seconds only" << std::endl;
}
...
void log_to_csv(std::ofstream &csv_file, double timestamp, double m7_pos,
                double m8_pos) {
  if (csv_file.is_open()) {
    csv_file << std::fixed << std::setprecision(6);
    csv_file << timestamp << "," << m7_pos << "," << m8_pos << ","
...
int main(int argc, char *argv[]) {
  ...
  if (frequency <= 0 || frequency > 1000) {
    std::cerr << "❌ Invalid frequency: " << frequency
              << " Hz. Must be between 0 and 1000 Hz." << std::endl;
    return 1;
  }
  ...
  if (enable_logging) {
    ...
    std::stringstream ss;
    ss << "wrist_motor_data_"
       << std::put_time(std::localtime(&time_t), "%Y%m%d_%H%M%S") << ".csv";
    csv_file.open(ss.str());
    if (csv_file.is_open()) {
      csv_file << "timestamp,m7_pos_rad,m8_pos_rad,m7_pos_deg,m8_pos_deg"
               << std::endl;
  ...
  try {
    ...
    // Enable debug mode for wrist component to see CAN frame details
    wrist.set_debug_mode(true);
    std::cout << "🔧 Wrist debug mode enabled - will show CAN frame details"
              << std::endl;
    ...
    const auto command_interval =
        std::chrono::duration<double>(1.0); // Send commands every 1 second
    ...
    while (keep_running) {
      ...
      if (duration > 0) {
        ...
        if (elapsed >= duration) {
          std::cout << "⏰ Time limit reached, stopping monitoring"
                    << std::endl;
          break;
        }
      }

      // Send periodic position requests to HT motors
      if (current_time - last_command_time >= command_interval) {
        std::cout << "📤 Sending position requests to HT motors..."
                  << std::endl;
