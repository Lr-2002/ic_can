...
// Default method
static TorquePredictionMethod current_method_ = TorquePredictionMethod::PINOCCHIO_URDF;
...
TorquePredictionMethod get_torque_prediction_method() {
    ...
    if (env_method) {
        ...
        if (method_str == "PINOCCHIO_URDF" || method_str == "0") {
            return TorquePredictionMethod::PINOCCHIO_URDF;
        } else if (method_str == "PURE_C_MATLAB" || method_str == "1") {
            return TorquePredictionMethod::PURE_C_MATLAB;
        } else if (method_str == "REGRESSOR_BASED" || method_str == "2") {
...
bool is_torque_method_available(TorquePredictionMethod method) {
    // For now, assume all methods are available if their respective
