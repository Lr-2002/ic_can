/**
 * @file ic_can_position_monitor.cpp
 * @brief Position Monitor using the complete IC_CAN system
 *
 * This program demonstrates real-time position monitoring for all 9 motors
 * using the properly integrated IC_CAN system with working dm-tools SDK.
 */
...
// Include the main IC_CAN system
...
void print_all_motor_positions(ic_can::IC_CAN& controller) {
    ...
    // Print header
    std::cout << "Joint | Motor | Type      | Position (rad) | Position (deg) | Velocity (rad/s) | Torque (Nm) | Updated" << std::endl;
    std::cout << "------|-------|-----------|----------------|----------------|-------------------|-------------|--------" << std::endl;
    ...
    for (int i = 0; i < 9; i++) {
        ...
        std::cout << std::setw(5) << (i + 1) << " | "
                  ...
                  << std::fixed << std::setw(11) << std::setprecision(3) << tau << " | "
                  << "✅" << std::endl;
    }
...
void test_high_frequency_control(ic_can::IC_CAN& controller) {
...
int main() {
    ...
    try {
        ...
        if (g_running) {
            test_high_frequency_control(*controller);
        }
