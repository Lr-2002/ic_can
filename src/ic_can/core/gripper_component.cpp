...
GripperComponent::GripperComponent()
    : MotorProtocolBase("GripperServo", 0x19, 0x19),
      min_openness_(DEFAULT_MIN_OPENNESS), max_openness_(DEFAULT_MAX_OPENNESS),
      max_speed_(DEFAULT_MAX_SPEED), max_force_(DEFAULT_MAX_FORCE) {

  debug_print("GripperComponent created for servo motor (m9)");
  std::cout << "   CAN Receive ID: 0x" << std::hex << 0x19 << std::dec
            << " (m9 feedback)" << std::endl;
}
...
bool GripperComponent::process_can_frame(const CANFrame &frame) {
  ...
  if (!is_valid_motor_id(motor_id)) {
    if (debug_enabled_) {
      debug_print("Invalid motor ID from CAN frame 0x" +
  ...
  if (!motor) {
    if (debug_enabled_) {
      debug_print("Motor " + std::to_string(motor_id) +
                  " not found for frame processing");
    }
    return false;
  }
  ...
  if (success && debug_enabled_) {
    ...
    oss << "Processed frame for motor " << motor_id << " (ID: 0x" << std::hex
        << frame.id << std::dec << ")";
    debug_print(oss.str());
  }
...
bool GripperComponent::set_servo_angle(double angle, double speed) {
  ...
  double clamped_angle =
      std::clamp(angle, -SERVO_ANGLE_RANGE / 2, SERVO_ANGLE_RANGE / 2);
  double clamped_speed = std::clamp(speed, 0.0, max_speed_);
...
double GripperComponent::angle_to_pulse_width(double angle) {
  ...
  return SERVO_MIN_PULSE_US +
         normalized_angle * (SERVO_MAX_PULSE_US - SERVO_MIN_PULSE_US);
}

double GripperComponent::pulse_width_to_angle(double pulse_width) {
  // Convert pulse width to angle
  double normalized = (pulse_width - SERVO_MIN_PULSE_US) /
...
void GripperComponent::update_gripper_state() {
  ...
  if (debug_enabled_) {
    debug_print("Updated state - openness: " +
                std::to_string(current_openness_.load()) +
                ", angle: " + std::to_string(angle) +
                "°, moving: " + (is_moving_.load() ? "yes" : "no"));
  }
}
...
void GripperComponent::debug_print(const std::string &message) {
  if (debug_enabled_) {
    auto timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(
                         ...
                         .count();
    std::cout << "[" << timestamp << "] [GRIPPER-SERVO] " << message
              << std::endl;
  }
}
