...
struct HTMotorFrame {
    ...
    // Tail (6 bytes)
    uint8_t tail[6] = {0x11, 0x00, 0x1F, 0x01, 0x13, 0x0D};
     ...
     */
    HTMotorFrame() {
        std::memset(pad, 0x50, sizeof(pad));
    }
     ...
     */
    void set_control_params(float position, float velocity, float torque, float kp_gain, float kd_gain) {
        pos = position;
        vel = velocity;
        torque = torque;
        kp = kp_gain;
