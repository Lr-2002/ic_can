...
TorquePredictorPinocchio::TorquePredictorPinocchio(const std::string& urdf_path)
    ...
    if (urdf_path_.empty()) {
        ...
        std::vector<std::string> possible_paths = {
            "/home/lr-2002/project/InstantCreation/ic_can/urdf/urdf/ic_arm_cnc.urdf",
    ...
    std::cout << "🔧 Initializing Pinocchio-based Torque Predictor..." << std::endl;
    if (!urdf_path_.empty()) {
        initialize(urdf_path_);
    } else {
        std::cout << "❌ No URDF file found. Please provide a valid path." << std::endl;
    }
}
...
bool TorquePredictorPinocchio::initialize(const std::string& urdf_path) {
    try {
        ...
        std::cout << "🦾 Model has " << model_.nv << " DOF and " << model_.njoints << " joints" << std::endl;
        ...
        std::cout << "❌ Failed to initialize Pinocchio model: " << e.what() << std::endl;
        initialized_ = false;
        return false;
    }
}
...
bool TorquePredictorPinocchio::validate_joint_config(const double q[6], const double dq[6], const double ddq[6]) {
...
bool TorquePredictorPinocchio::predict_torques(const double q[6], const double dq[6], const double ddq[6],
    ...
    try {
        ...
        Eigen::VectorXd torques = pinocchio::rnea(model_, data_, q_config_, v_config_, a_config_);
        ...
        // Gravity component
        Eigen::VectorXd G_component = pinocchio::computeGeneralizedGravity(model_, data_, q_config_);
...
bool TorquePredictorPinocchio::predict_total_torque(const double q[6], const double dq[6], const double ddq[6],
                                                    double total_torque[6]) {
    if (!initialized_) {
        std::cout << "❌ Pinocchio predictor not initialized" << std::endl;
        return false;
...
bool TorquePredictorPinocchio::predict_gravity_torque(const double q[6], double gravity_torque[6]) {
    if (!initialized_) {
        std::cout << "❌ Pinocchio predictor not initialized" << std::endl;
        return false;
    }
    ...
    try {
        ...
        Eigen::VectorXd G_torques = pinocchio::computeGeneralizedGravity(model_, data_, q_config_);
        ...
        std::cout << "❌ Error computing gravity torques: " << e.what() << std::endl;
        return false;
    }
}

bool TorquePredictorPinocchio::compute_mass_matrix(const double q[6], Eigen::MatrixXd& mass_matrix) {
    if (!initialized_) {
        std::cout << "❌ Pinocchio predictor not initialized" << std::endl;
        return false;
    }
    ...
    try {
        ...
        mass_matrix.triangularView<Eigen::StrictlyLower>() = mass_matrix.transpose();
...
bool TorquePredictorPinocchio::compute_coriolis_matrix(const double q[6], const double dq[6],
    ...
    try {
        ...
        std::cout << "❌ Error computing Coriolis matrix: " << e.what() << std::endl;
        return false;
    }
}

void TorquePredictorPinocchio::print_torque_breakdown(const double q[6], const double dq[6], const double ddq[6]) {
    if (!initialized_) {
        std::cout << "❌ Pinocchio predictor not initialized" << std::endl;
        return;
    }
    ...
    if (predict_torques(q, dq, ddq, M_torque, C_torque, G_torque, total_torque)) {
        ...
        std::cout << std::string(60, '-') << std::endl;
        std::cout << "Magnitudes: | "
                  << std::setw(8) << std::sqrt(M_mag) << " | "
                  << std::setw(8) << std::sqrt(C_mag) << " | "
                  << std::setw(8) << std::sqrt(G_mag) << " | "
                  << std::setw(8) << std::sqrt(total_mag) << std::endl;
    }
    std::cout << std::string(60, '=') << std::endl;
}
...
bool TorquePredictorPinocchio::get_joint_limits(double lower[6], double upper[6]) {
    if (!initialized_) {
        return false;
    }
    ...
    std::cout << "⚠️  Joint limits not implemented yet (requires URDF parsing)" << std::endl;
    return false;
}
